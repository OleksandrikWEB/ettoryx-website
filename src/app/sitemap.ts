...
function getPriority(path: string): number {
  ...
  if (path.startsWith("/career/")) return 0.7;
  return 0.6;
}

export default function sitemap(): MetadataRoute.Sitemap {
  const staticPaths = ["", "/about", "/services", "/products", "/portfolio", "/career", "/team", "/contacts", "/privacy-policy"];
  const dynamicPaths = [
    ...cms.getServiceSlugs().map((s) => `/services/${s}`),
    ...cms.getCaseStudySlugs().map((s) => `/portfolio/${s}`),
    ...cms.getJobSlugs().map((s) => `/career/${s}`),
  ];
  ...
  return routing.locales.flatMap((locale) =>
    all.map((path) => ({
      ...
      changeFrequency: (path === "" || path === "/services" || path === "/portfolio" ? "weekly" : "monthly") as "weekly" | "monthly",
