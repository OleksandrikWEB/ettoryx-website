...
export function SectionHeader({
  ...
  return (
    ...
    >
      ...
      <h2 className="mt-4 font-display text-h3 text-ink-primary sm:text-h2">
        {title}
      </h2>
      {subtitle && (
        <p className="mt-4 text-body-lg text-ink-secondary">{subtitle}</p>
      )}
    </div>
  );
}
...
export function ProductCard({
  ...
  return (
    ...
    >
      ...
      <p className="mt-4 font-display text-h6 text-gold-gradient">
    ...
    >
      <div className="overflow-hidden rounded-2xl border border-line transition-[border-color,box-shadow] duration-300 group-hover:border-gold/60 group-hover:shadow-gold">
        ...
        <div className="p-6">
          ...
          <h3 className="mt-4 font-display text-h5 text-ink-primary">
            {item.title[locale]}
          </h3>
          <p className="mt-2 font-display text-h6 text-gold-gradient">
            {item.result[locale]}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="group flex h-full flex-col" data-testid={`team-card-${member.slug}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-display text-h6 text-ink-primary">
            {member.name}
          </h3>
